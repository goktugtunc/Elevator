...
flags and the shared idempotent `record_interaction` helper used by discover / offers / listing views.
...
* capital listings carry the on-chain terms the offers inherit (amount, base asset, duration, max loss);
  the base asset must be an active `is_base_allowed` asset of the configured network;
...
async def default_base_asset(db: AsyncSession, settings: Settings) -> Asset:
    """The network's default base asset (`settings.default_base_asset_code`, must be allow-listed as base)."""
    q = (
        select(Asset)
        .where(
            Asset.network == settings.stellar_network,
            Asset.code == settings.default_base_asset_code,
    ...
    if asset is None:
        raise ValidationError(
            f"Default base asset {settings.default_base_asset_code} is not configured for {settings.stellar_network}",
...
async def resolve_base_asset(db: AsyncSession, settings: Settings, asset_id: uuid.UUID | None) -> Asset:
    """`asset_id` -> Asset that may hold escrowed capital (active, base-allowed, this network)."""
    if asset_id is None:
    ...
    if not asset.is_active or not asset.is_base_allowed:
        raise ValidationError(
            f"{asset.code} cannot be used as a base asset", code="asset_not_base", details={"asset_id": str(asset.id)}
        )
    return asset
...
async def create_listing(db: AsyncSession, settings: Settings, owner: User, data: ListingCreateIn) -> Listing:
    ...
    if kind is ListingKind.capital:
        ...
        listing.amount = Decimal(data.amount)
...
def listing_ref_bytes(listing: Listing) -> bytes:
    """32 raw bytes tying an on-chain reservation to this listing (indexing only)."""
    from app.services.agreements import compute_listing_ref

    return bytes.fromhex(compute_listing_ref(listing.id))


async def build_reserve_tx(
    db: AsyncSession, settings: Settings, soroban: Any, owner: User, listing_id: uuid.UUID
) -> tuple[Listing, Any, Any]:
    """Unsigned `reserve(...)` that moves the listing's capital into the vault.

    This is what publishes a capital listing: until the customer signs it the listing is a draft,
    so nobody can advertise capital they do not hold. Returns `(listing, unsigned, pending)`.
    """
    ...
    if not owner.stellar_address:
        raise StateError("Connect a wallet first", code="wallet_required")

    # Freighter imzaladığı işlemi kendisi de yayınlayabiliyor, yani `/tx/submit`
    # hiç çağrılmasa bile para zincire gidebiliyor. Bu yüzden "tekrar dene"
    # imzalanmış ama sonucu henüz bilinmeyen bir yatırmanın üstüne ikinci bir
    # yatırma bindirmemeli: süresi dolmamış bir `reserve` varken yenisi
    # üretilmez, kullanıcı önce onun sonucunu görür.
    in_flight = (
        await db.execute(
            select(PendingTransaction).where(
                PendingTransaction.user_id == owner.id,
                PendingTransaction.kind == PendingTxKind.reserve,
                ...
                PendingTransaction.payload["context"]["listing_id"].astext == str(listing.id),
            )
        )
    ).scalars().first()
    if in_flight is not None:
        raise StateError(
            "A deposit for this listing is already waiting to be signed or confirmed",
    ...
    if asset is None or not asset.contract_id:
        raise ValidationError("base asset has no contract id", code="asset_not_supported")

    unsigned = await soroban.build_reserve(
        owner.stellar_address,
        asset.contract_id,
        to_stroops(listing.amount, asset.decimals),
        listing_ref_bytes(listing),
    )
    pending = await record_pending(
        db,
        owner,
        PendingTxKind.reserve,
        None,
        unsigned,
        {"listing_id": str(listing.id), "amount": str(listing.amount), "asset_code": asset.code},
    )
...
async def build_release_tx(
    ...
    """Unsigned `release(...)` that hands the still-locked capital back to the customer."""
    from app.services.agreements import record_pending

    listing = await get_owned_listing(db, owner, listing_id, for_update=True)
    if listing.reservation_id is None:
        raise StateError("This listing has no locked capital", code="listing_not_reserved")
    if not listing.reserved_amount or listing.reserved_amount <= 0:
        raise StateError(
            "The locked capital is already used or released", code="listing_capital_spent"
        )
    if not owner.stellar_address:
        raise StateError("Connect a wallet first", code="wallet_required")

    # 0 = "release whatever is left", so a concurrent draw cannot make this fail.
    unsigned = await soroban.build_release(owner.stellar_address, int(listing.reservation_id), 0)
    pending = await record_pending(
        db,
        owner,
        PendingTxKind.release,
        None,
        unsigned,
        {"listing_id": str(listing.id), "reservation_id": int(listing.reservation_id)},
    )
    return listing, unsigned, pending
...
async def set_status(db: AsyncSession, owner: User, listing_id: uuid.UUID, action: ListingStatusAction) -> Listing:
    """pause: active -> paused; resume: paused -> active; close: active|paused -> closed (terminal)."""
    listing = await get_owned_listing(db, owner, listing_id, for_update=True)
    ...
    if target is ListingStatus.closed and listing.reserved_amount and listing.reserved_amount > 0:
        raise StateError(
            ...
            details={"reservation_id": listing.reservation_id, "reserved_amount": str(listing.reserved_amount)},
        )
