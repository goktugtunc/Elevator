"""Listings (Figma 5a-5d, 9a): CRUD, mine, detail (+offers for the owner), pause / resume / close."""
...
from app.api_deps import DB, CurrentUser, OptionalUser, SettingsDep, SorobanDep
...
async def create_listing(body: ListingCreateIn, user: CurrentUser, db: DB, settings: SettingsDep) -> ListingOut:
    """Customer -> capital listing (5a), trader -> service listing (5b)."""
...
async def my_listings(
    ...
    db: DB,
    page: PageDep,
    status: Annotated[ListingStatus | None, Query(description="active | paused | closed")] = None,
) -> Page[ListingOut]:
...
async def reserve_listing_capital(
    listing_id: uuid.UUID, user: CurrentUser, db: DB, settings: SettingsDep, soroban=SorobanDep
) -> UnsignedTxOut:
    """Unsigned `reserve(...)`: the customer moves this listing's capital into the vault.

    A capital listing is a draft until this is signed and confirmed, so the amount on a public
    listing is always money the owner actually holds. Sign it and `POST /tx/submit`.
    """
    _, unsigned, pending = await listings_service.build_reserve_tx(db, settings, soroban, user, listing_id)
    return agreements_service.unsigned_out(pending, unsigned, user.stellar_address or "")
...
async def release_listing_capital(
    ...
    """Unsigned `release(...)`: hands back whatever of this listing's capital is still locked."""
    _, unsigned, pending = await listings_service.build_release_tx(db, settings, soroban, user, listing_id)
    return agreements_service.unsigned_out(pending, unsigned, user.stellar_address or "")
