...
use elevator_mock_router::{MockRouter, MockRouterClient};

use crate::{
    math, Activated, Agreement, Cancelled, Claimed, Config, ConfigChanged, DataKey, Error,
    Opened, Proposed, ReservationStatus, Settled, Status, Terms, TokenInfo, TokenSet,
    ElevatorVault, ElevatorVaultClient, Traded, Unliquidated, MAX_TOKENS,
    SETTLE_GRACE_SECS, TTL_EXTEND_TO,
};
...
impl Ctx {
    fn client(&self) -> ElevatorVaultClient<'_> {
        ElevatorVaultClient::new(&self.env, &self.vault)
    }
...
// ---------------------------------------------------------------------------
// Reservations — capital locked up front, at listing time
// ---------------------------------------------------------------------------

#[test]
fn reserve_locks_capital_and_release_gives_it_back() {
    let c = setup();
    let cl = c.client();
    let before = c.bal(&c.usdc, &c.customer);
    let vault_before = c.bal(&c.usdc, &c.vault);

    let rid = cl.reserve(&c.customer, &c.usdc, &PRINCIPAL, &BytesN::random(&c.env));
    assert_eq!(c.bal(&c.usdc, &c.customer), before - PRINCIPAL);
    assert_eq!(c.bal(&c.usdc, &c.vault), vault_before + PRINCIPAL);

    let res = cl.get_reservation(&rid);
    assert_eq!(res.customer, c.customer);
    assert_eq!(res.token, c.usdc);
    assert_eq!(res.amount, PRINCIPAL);
    assert_eq!(res.original, PRINCIPAL);
    assert_eq!(res.status, ReservationStatus::Open);

    // A partial release leaves the rest locked.
    let quarter = PRINCIPAL / 4;
    assert_eq!(cl.release(&rid, &quarter), quarter);
    let res = cl.get_reservation(&rid);
    assert_eq!(res.amount, PRINCIPAL - quarter);
    assert_eq!(res.original, PRINCIPAL);
    assert_eq!(res.status, ReservationStatus::Open);

    // `amount <= 0` hands back everything that is left.
    assert_eq!(cl.release(&rid, &0), PRINCIPAL - quarter);
    assert_eq!(c.bal(&c.usdc, &c.customer), before);
    assert_eq!(cl.get_reservation(&rid).status, ReservationStatus::Released);
    assert_eq!(cl.try_release(&rid, &0), Err(Ok(Error::ReservationClosed)));
}
...
fn reserve_validation() {
    let c = setup();
    let cl = c.client();
    let r = BytesN::random(&c.env);
    // allow-listed, but not as a base token
    assert_eq!(
        cl.try_reserve(&c.customer, &c.eurc, &PRINCIPAL, &r),
        Err(Ok(Error::TokenNotAllowed))
    );
    assert_eq!(
        cl.try_reserve(&c.customer, &c.usdc, &0, &r),
        Err(Ok(Error::InvalidAmount))
    );
    assert_eq!(cl.try_get_reservation(&7), Err(Ok(Error::ReservationNotFound)));

    cl.set_paused(&true);
    assert_eq!(
        cl.try_reserve(&c.customer, &c.usdc, &UNIT, &r),
        Err(Ok(Error::Paused))
    );
}
...
fn release_works_while_paused() {
    // A pause must never trap a customer's own money.
    let c = setup();
    let cl = c.client();
    let before = c.bal(&c.usdc, &c.customer);
    let rid = cl.reserve(&c.customer, &c.usdc, &PRINCIPAL, &BytesN::random(&c.env));
    cl.set_paused(&true);
    assert_eq!(cl.release(&rid, &0), PRINCIPAL);
    assert_eq!(c.bal(&c.usdc, &c.customer), before);
}

#[test]
fn open_reserved_draws_the_principal_without_touching_the_wallet() {
    let c = setup();
    let cl = c.client();
    let rid = cl.reserve(&c.customer, &c.usdc, &PRINCIPAL, &BytesN::random(&c.env));
    let after_reserve = c.bal(&c.usdc, &c.customer);

    let id = cl.open_reserved(&c.customer, &c.terms(), &rid);
    // The capital was already inside the vault, so no second transfer.
    assert_eq!(c.bal(&c.usdc, &c.customer), after_reserve);
    assert_eq!(c.vault_balance(id, &c.usdc), PRINCIPAL);
    assert_eq!(c.agreement(id).status, Status::Funded);
...
#[test]
fn fund_reserved_activates_a_proposed_agreement() {
    let c = setup();
    let cl = c.client();
    let rid = cl.reserve(&c.customer, &c.usdc, &PRINCIPAL, &BytesN::random(&c.env));
    let after_reserve = c.bal(&c.usdc, &c.customer);
    ...
    let id = cl.propose(&c.trader, &c.terms());
    cl.fund_reserved(&id, &rid);

    assert_eq!(c.bal(&c.usdc, &c.customer), after_reserve);
    assert_eq!(c.agreement(id).status, Status::Active);
    assert_eq!(c.vault_balance(id, &c.usdc), PRINCIPAL);
    assert_eq!(cl.get_reservation(&rid).status, ReservationStatus::Consumed);
    // The reservation is spent; a second agreement cannot reuse it.
    let id2 = cl.propose(&c.trader, &c.terms());
    assert_eq!(cl.try_fund_reserved(&id2, &rid), Err(Ok(Error::ReservationClosed)));
}

#[test]
fn reserved_funding_rejects_a_reservation_that_does_not_fit() {
    let c = setup();
    let cl = c.client();
    c.mint(&c.usdc, &c.customer, PRINCIPAL); // room for a second reservation
    c.mint(&c.xlm, &c.customer, PRINCIPAL);
    let id = cl.propose(&c.trader, &c.terms()); // base token is USDC

    let wrong_token = cl.reserve(&c.customer, &c.xlm, &PRINCIPAL, &BytesN::random(&c.env));
    assert_eq!(
        cl.try_fund_reserved(&id, &wrong_token),
        Err(Ok(Error::ReservationMismatch))
    );

    c.mint(&c.usdc, &c.stranger, PRINCIPAL);
    let someone_else = cl.reserve(&c.stranger, &c.usdc, &PRINCIPAL, &BytesN::random(&c.env));
    assert_eq!(
        cl.try_fund_reserved(&id, &someone_else),
        Err(Ok(Error::ReservationMismatch))
    );

    let too_small = cl.reserve(&c.customer, &c.usdc, &(PRINCIPAL / 2), &BytesN::random(&c.env));
    assert_eq!(
        cl.try_fund_reserved(&id, &too_small),
        Err(Ok(Error::ReservationInsufficient))
    );

    assert_eq!(
        cl.try_fund_reserved(&id, &9_999),
        Err(Ok(Error::ReservationNotFound))
    );
}

#[test]
fn a_reservation_bigger_than_the_principal_keeps_the_rest_locked() {
    let c = setup();
    let cl = c.client();
    c.mint(&c.usdc, &c.customer, PRINCIPAL); // 2 x PRINCIPAL on hand
    let rid = cl.reserve(&c.customer, &c.usdc, &(2 * PRINCIPAL), &BytesN::random(&c.env));
    ...
    // What is left is still the customer's to take back.
    assert_eq!(cl.release(&rid, &0), PRINCIPAL);
    assert_eq!(cl.get_reservation(&rid).status, ReservationStatus::Released);
}

#[test]
fn reserved_agreement_settles_and_pays_the_customer() {
    let c = setup();
    let cl = c.client();
    let before = c.bal(&c.usdc, &c.customer);
    ...
    c.env.ledger().set_timestamp(START_TS + DURATION + 1);
    cl.settle(&id, &c.customer, &vec![&c.env]);

    // No trades happened, so the principal comes back whole and no fee is due.
    assert_eq!(c.agreement(id).status, Status::Settled);
    assert_eq!(c.bal(&c.usdc, &c.customer), before);
}

#[test]
fn cancelling_a_reserved_agreement_refunds_the_wallet() {
    // Cancel pays the principal back to the customer's wallet rather than
    // into the reservation it came from; the reservation stays spent.
    let c = setup();
    let cl = c.client();
    let before = c.bal(&c.usdc, &c.customer);
    let rid = cl.reserve(&c.customer, &c.usdc, &PRINCIPAL, &BytesN::random(&c.env));
...
#[test]
fn reservation_ids_are_their_own_sequence() {
    let c = setup();
    let cl = c.client();
    c.mint(&c.usdc, &c.customer, PRINCIPAL);
    assert_eq!(cl.next_reservation_id(), 1);
    let a = cl.reserve(&c.customer, &c.usdc, &UNIT, &BytesN::random(&c.env));
    // An agreement in between must not shift the reservation sequence.
    cl.propose(&c.trader, &c.terms());
    let b = cl.reserve(&c.customer, &c.usdc, &UNIT, &BytesN::random(&c.env));
    assert_eq!((a, b), (1, 2));
    assert_eq!(cl.next_reservation_id(), 3);
}
