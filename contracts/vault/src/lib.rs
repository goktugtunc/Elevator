//! Elevator vault — a singleton escrow contract for customer/trader
...
contractmeta!(key = "desc", val = "Elevator escrow vault");

#[contract]
pub struct ElevatorVault;
...
/// Moves `ag.terms.principal` out of a reservation and into the agreement.
/// No token transfer happens: `reserve` already pulled the capital into the
/// contract, so this is pure accounting. The reservation must belong to the
/// same customer and hold the agreement's base token.
fn draw_reservation(env: &Env, reservation_id: u64, ag: &Agreement) -> Result<(), Error> {
    let mut res = storage::get_reservation(env, reservation_id)?;
    if res.status != ReservationStatus::Open {
        return Err(Error::ReservationClosed);
    }
    if res.customer != ag.terms.customer || res.token != ag.terms.base_token {
        return Err(Error::ReservationMismatch);
    }
    let remaining = res
        .amount
        .checked_sub(ag.terms.principal)
    ...
    storage::set_reservation(env, &res);
    storage::set_balance(env, ag.id, &ag.terms.base_token, ag.terms.principal);
    ReservationDrawn {
        id: reservation_id,
        agreement_id: ag.id,
        amount: ag.terms.principal,
        remaining,
    }
    .publish(env);
    Ok(())
...
impl ElevatorVault {
    ...
    /// Customer locks capital up front so a listing is backed by real funds.
    /// The tokens move into the contract immediately and stay there until an
    /// agreement draws on them or `release` hands them back. Returns the
    /// reservation id.
    pub fn reserve(
        env: Env,
        customer: Address,
        ...
        let info = storage::get_token_info(&env, &token);
        if !(info.allowed && info.is_base) {
            return Err(Error::TokenNotAllowed);
        }
        TokenClient::new(&env, &token).transfer(&customer, &env.current_contract_address(), &amount);
        let id = storage::take_next_reservation_id(&env)?;
        let res = Reservation {
            id,
    ...
    /// Customer takes locked capital back out (the listing is closed, or its
    /// amount is lowered). `amount <= 0` releases everything still held.
    /// Works while paused: a pause must never trap a customer's money.
    /// Returns the amount sent back.
    pub fn release(env: Env, id: u64, amount: i128) -> Result<i128, Error> {
        storage::extend_instance(&env);
        let mut res = storage::get_reservation(&env, id)?;
        res.customer.require_auth();
        if res.status != ReservationStatus::Open {
            return Err(Error::ReservationClosed);
        }
        let out = if amount <= 0 { res.amount } else { amount };
        if out <= 0 {
            return Err(Error::InvalidAmount);
        }
        if out > res.amount {
        ...
        let remaining = res.amount.checked_sub(out).ok_or(Error::Overflow)?;
        TokenClient::new(&env, &res.token).transfer(
            &env.current_contract_address(),
            &res.customer,
            &out,
        );
    ...
    pub fn get_reservation(env: Env, id: u64) -> Result<Reservation, Error> {
        storage::get_reservation(&env, id)
    }

    /// Id the next `reserve` will receive.
    pub fn next_reservation_id(env: Env) -> Result<u64, Error> {
        Ok(storage::peek_next_reservation_id(&env))
    }
    ...
    /// Same as `open`, but the principal comes out of a reservation the
    /// customer already locked instead of their wallet. The capital is
    /// already inside the contract, so no transfer happens here.
    pub fn open_reserved(
        ...
        let id = storage::take_next_id(&env)?;
        let ag = new_agreement(&env, id, terms, Status::Funded, customer);
        draw_reservation(&env, reservation_id, &ag)?;
        storage::set_agreement(&env, &ag);
        Opened {
            id,
            trader: ag.terms.trader.clone(),
    ...
    /// Same as `fund`, but draws on a reservation the customer already
    /// locked instead of their wallet.
    pub fn fund_reserved(env: Env, id: u64, reservation_id: u64) -> Result<(), Error> {
        storage::extend_instance(&env);
        let mut ag = storage::get_agreement(&env, id)?;
        ag.terms.customer.require_auth();
        if storage::get_config(&env)?.paused {
