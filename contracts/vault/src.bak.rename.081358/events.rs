...
/// Customer locked capital up front (listing time).
#[contractevent]
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct Reserved {
    #[topic]
...
/// Customer took locked capital back out. `remaining` is what the
/// reservation still holds afterwards.
#[contractevent]
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct Released {
    #[topic]
...
/// An agreement was funded out of a reservation instead of the customer's
/// wallet. No token transfer happens: the capital was already in the vault.
#[contractevent]
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ReservationDrawn {
    #[topic]
    pub id: u64,
    #[topic]
